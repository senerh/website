...
Similarly, when using Guice there is [a scope `ScenarioScoped`](https://github.com/cucumber/cucumber-jvm/tree/main/cucumber-guice#using-scope-annotations) or [a scope `ScenarioScope`](https://docs.quarkiverse.io/quarkus-cucumber/dev/index.html#_scenarioscope) with Quarkus that should always be used on step definition classes.
...
<dependency>
    ...
    <version>1.2.0</version><!-- check the last version on https://github.com/quarkiverse/quarkus-cucumber/releases -->
</dependency>
```

Or, if you are using Gradle, add:
```
compile group: 'io.quarkiverse.cucumber', name: 'quarkus-cucumber', version: '1.2.0'
```

The documentation is available on [Quarkiverse](https://docs.quarkiverse.io/quarkus-cucumber/dev/index.html).
